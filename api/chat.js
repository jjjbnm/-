...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const item = JSON.stringify({ from: me, to: other, message, createdAt: new Date().toISOString() }); await redis('lpush', key(me, other), item); await redis('ltrim', key(me, other), '0', '199'); return res.status(201).json({ message: JSON.parse(item) });
